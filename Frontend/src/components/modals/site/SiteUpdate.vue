<template>
 <div v-if="showFlag" class="site-update" >
    <div class="title">수정</div>
    <div class="info-box">
      <div>지도명</div>
      <input class="site-name" id="site-name" v-model="siteInfo.siteName">
      ...
      <div class="upload-site">
        ...
        <input type="file" id="img-file" style="display: none;" accept="image/png" @change="uploadFile">
      </div>
    ...
    <div class="button-box">
      ...
      <button @click="siteUpdate" >확인</button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup () {
    ...
    onMounted(() =>{
      proxy._eventBus.on('showSiteUpdateModal', (params) =>{
        showFlag.value = params.flag;
        if(params.siteInfo != null){
          siteInfo.key = params.siteInfo.key;
          siteInfo.siteName = params.siteInfo.siteName;
          siteInfo.siteInfo = params.siteInfo.siteInfo;
          siteInfo.imgFile.name = params.siteInfo.imgFile.name;
          siteInfo.imgFile.file = params.siteInfo.imgFile.file;
          siteInfo.yamlFile.name = params.siteInfo.yamlFile.name;
          siteInfo.yamlFile.file = params.siteInfo.yamlFile.file;
          
          //siteInfo.siteImageFileName = params.siteInfo.siteImageFileName;
          //siteInfo.siteImageUrl = params.siteInfo.siteImageUrl;
    ...
    const uploadFile = (e) => {
      ...
      switch(e.target.id){
        case 'img-file':
        if(extention.search('png') >= 0 || extention.search('pgm') >= 0 || extention.search('jpg') >= 0){          
    ...
    const siteUpdate = () =>{
      ...
      API.post('/upload/', imgData, { 
        ...
        if(yamlUploadInfo != ''){
          ...
          API.put('/site-lists/'+siteInfo.key,  {
      ...
      API.post('/upload/', yamlData, { 
        ...
        if(imgUploadInfo != ''){
          ...
          API.post('/site-lists/'+siteInfo.key,  {
              site_name: siteInfo.siteName,
              site_info: siteInfo.siteInfo,
              robot_list:checkRobotList,
              first_uploaded_by: sessionStorage.getItem('userId'),
    ...
    const getRobotList = () =>{
      ...
      //get robot list
      API.get('/robot-lists-in-site?site='+ siteInfo.siteName).then(response => {
