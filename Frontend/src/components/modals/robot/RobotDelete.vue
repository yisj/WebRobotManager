<template>
  <div v-if="showFlag" class="robot-delete" >
    <div class="title">선택한 로봇를 삭제하겠습니까?</div>

    <div class="button-box">
      <button @click="cancel" >취소</button>
      <button @click="deleteRobot" >확인</button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup () {
    ...
    onMounted(() => {
      proxy._eventBus.on('showRobotDeleteModal', (params)=>{
        console.log(params);
    ...
    const deleteRobot = () =>{
      // TODO
      //delete robot
      API.delete('/robot-lists/' + key.value).then(
        response => {
          showFlag.value = false;
        },
        error => {
